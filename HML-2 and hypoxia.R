...
HML2_proviruses = rownames(big_data)[!rownames(big_data) %in% "sample_size"]
...
counts_filelist = list.files(pattern = "*TPM_total.csv") #save data as a list
counts_filelist
...
for (i in (1:length(counts_datalist))) {
  Tissue = stringr::str_replace((names(counts_datalist)[[i]]), '_hypoxia_HML2_TPM.csv', '')
  Tissue
  #read in counts data
  df <- counts_datalist[[i]]
  rownames(df) = df$X
