...
File_Type="*_Telescope_outputtelescope-telescope_report.tsv"
...
colnames(Telescope_Counts) <- c("Gene_names", unlist(lapply(strsplit(filelist, "_Telescope_outputtelescope-telescope_report.tsv"), function(x) x[[1]])) )
head(Telescope_Counts)
rownames(Telescope_Counts) = Telescope_Counts$Gene_names
Telescope_Counts = Telescope_Counts[,-1]
head(Telescope_Counts)

write.csv(Telescope_Counts, file = paste('Telescope_output', '.csv', sep="_"))
