...
for (i in (1:length(HML2_datalist))) {
  ...
  ERV_Counts_modT_hardy = join(ERV_Counts_modT, meta_updated, by = "Run", type = "left")
...
test = big_data[c("Run","HML2_Sum", "tissue", "DTHHRDY")]
head(test)
table(test$DTHHRDY)
#0    1    2    3    4 
#5256  459 2293  561  966 
...
#get number of doners within each Hardy score for each tissue
...
write.csv(big_data_tissue_final,paste(Demographics,"Hardy_score_distribution_for_HML2_expression_differences_08142020.csv",sep="/"))
...
p<-ggplot(SpecTiss, aes(x=tissue, y=HML2_Sum, fill = DTHHRDY)) + geom_boxplot()+
...
p + stat_n_text(angle = 90, size=3) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(path=Hardy, filename = "Differences in HML-2 expression in regards to age in GTEx, zoom in.png")
