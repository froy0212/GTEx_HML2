...
binMatrix_MotifScan
#binMatrix_MotifScan[binMatrix_MotifScan >1] <- 1
...
ltrdriven <- binMatrix_MotifScan_df[c(24,26,37,48,53,2,9,10,32),]
t_ltrdriven <- t(ltrdriven)
ltr.pv <-pvclust(t_ltrdriven, nboot = 1000, method.hclust="ward.D2")
pdf("HML2_LTRMotif_Cluster_LTRdriven.pdf", width = 15)
plot(ltr.pv, print.pv =1, print.num=FALSE, col.pv=c(si=1, au=1, bp=1))
dev.off()
